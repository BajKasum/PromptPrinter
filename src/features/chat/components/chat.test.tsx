...
describe("Chat", () => {
  ...
  // Audit 23.09.2026: the picker (QA F-3) is gone again. Nobody used it and the
  // model ignored the chosen tool anyway; Finn asks about the tool in
  // conversation instead.
  describe("no target tool", () => {
    it("offers no picker and sends no target", async () => {
      mockStreamingFetch(["ok"], { conversationId: "conv-1" });
      render(<Chat />);

      expect(screen.queryByRole("button", { name: /Ziel-Tool/ })).not.toBeInTheDocument();
      await userEvent.type(screen.getByRole("textbox"), "Hi");
      await userEvent.click(screen.getByRole("button", { name: /Senden/ }));
