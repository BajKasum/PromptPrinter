...
describe("effectiveLimits", () => {
  ...
  // (2026-07-29: free 200→50, pro 2000→400) look like a regression in a test
  // that was never about the values.
...
describe("marketed plans match enforced limits", () => {
  ...
  it("keeps Pro's price and the free tier's zero price in sync with the config", async () => {
    const { PLANS, PRO_PRICE_EUR } = await import("@/lib/pricing");
    expect(PLANS.find((p) => p.name === "Free")?.price).toBe("0 €");
    expect(PLANS.find((p) => p.name === "Pro")?.price).toBe(`${PRO_PRICE_EUR} €`);
  });
});
